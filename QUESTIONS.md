...
| Q | Status |
|---|---|
| **Q1** | **CLOSED 1 Aug by R11 landing.** The cap this asked about no longer exists in the form it asked about: canon now records `--amb-alpha` at 20% light / 30% dark, raised from 8%/14% because at 8% the identity hue sat below the just-noticeable threshold. The assumption the question proposed — that the cap governs atmosphere, and a PetalPlate is a bounded imagery object that may run stronger while never sitting behind body copy — is unchanged and now consistent with the recorded values. No decision needed |
...
| **Q19a** | **CLOSED 1 Aug — the premise was wrong.** The six-hue rhythm is not *retired*; canon's identity block states outright that a page with no `data-identity` keeps the positional six-hue rhythm, "so the homepage and the editorial surfaces are unaffected". The homepage carries no `data-identity` and measures amb-1 216°, amb-2 180°, amb-3 325°, amb-4 133° — that is the sanctioned behaviour, not a leftover. R11 governs identity-bearing pages and deliberately does not reach this one |
| **Q19b** | **STILL NEEDS SUMEET.** Three dark bands render in light theme on the homepage — `#place`, `#start` and `footer`, all `rgb(15,15,15)` — against §5's two-dark-band ceiling. The question is whether footer chrome counts against the ceiling. Measured by computed background, not by class. I will not resolve this under S3: dropping a band is a visible design change to the homepage's closing rhythm, and reading the ceiling as excluding the footer is a canon amendment. Either way it is your call |
